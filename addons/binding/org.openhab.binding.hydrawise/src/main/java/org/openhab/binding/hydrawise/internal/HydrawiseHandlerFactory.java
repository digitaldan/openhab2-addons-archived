...
import org.openhab.binding.hydrawise.internal.discovery.HydrawiseControllerDiscoveryClient;
import org.openhab.binding.hydrawise.internal.discovery.HydrawiseRelaysAndSensorDiscoveryClient;
...
public class HydrawiseHandlerFactory extends BaseThingHandlerFactory {
    ...
    protected @Nullable ThingHandler createHandler(Thing thing) {
        ...
        if (HYDRAWISE_ACCOUNT_THING_TYPE.equals(thingTypeUID)) {
            HydrawiseAccountHandler accountHandler = new HydrawiseAccountHandler((Bridge) thing);
            ...
            this.discoveryServiceRegistrations.put(accountHandler.getThing().getUID(), bundleContext
                    .registerService(HydrawiseControllerDiscoveryClient.class.getName(), discovery, new Hashtable<>()));
            return accountHandler;
        }
        if (HYDRAWISE_CONTROLLER_THING_TYPE.equals(thingTypeUID)) {
            HydrawiseControllerHandler controllerHandler = new HydrawiseControllerHandler((Bridge) thing);
            HydrawiseRelaysAndSensorDiscoveryClient discovery = new HydrawiseRelaysAndSensorDiscoveryClient(
                    controllerHandler);
            this.discoveryServiceRegistrations.put(controllerHandler.getThing().getUID(), bundleContext.registerService(
                    HydrawiseRelaysAndSensorDiscoveryClient.class.getName(), discovery, new Hashtable<>()));
    ...
    protected void removeHandler(ThingHandler thingHandler) {
        if (thingHandler instanceof HydrawiseAccountHandler || thingHandler instanceof HydrawiseControllerHandler) {
