...
public class STTModel {
    ...
    private void evaluateErrorCode(int errorCode) {
        ...
        if (code != Error_Codes.ERR_OK) {
            throw new RuntimeException("Error: " + impl.ErrorCodeToErrorMessage(errorCode) + " (0x"
     ...
     *
     *        Words that don't occur in the scorer (e.g. proper nouns) or strings that contain spaces won't be taken
     *        into account.
     *
     * @param word
     * @param boost Positive value increases and negative reduces chance of a word occuring in a transcription.
